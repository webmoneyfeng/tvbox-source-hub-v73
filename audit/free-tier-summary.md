...
- dist：17254 files；8321800909 bytes
- PASS/WARN/FAIL：6/1/0

## 分项
- PASS；github_repo；public；公共仓库，GitHub Actions 标准 runner 按免费优先方案使用。
- PASS；github_actions_schedule；120/month；定时刷新频率按免费优先控制，保留人工发布余量。
- PASS；cloudflare_pages_files；17254；dist 静态快照采用打包文件，不采用每个节目一个文件。
- WARN；cloudflare_pages_size；8321800909 bytes；dist 总体积处于轻量级静态分发范围。
